...
export default function RegistrationModal({ isOpen, onClose }: RegistrationModalProps) {
  ...
  // 1. Recalculate price when category changes
  useEffect(() => {
    const isEB = isEarlyBirdActive();
    const price = isEB ? selectedKategori.hargaEarlyBird : selectedKategori.hargaReguler;
    setHargaDasar(price);
  ...
  const handleSubmitRegistration = async () => {
    ...
    try {
      ...
      setError("Koneksi server terputus. Pendaftaran gagal dikirim.");
    } finally {
            ...
            <div className="text-center py-6 space-y-6 animate-scaleIn">
              ...
              <div className="text-xs text-slate-600 space-y-1 max-w-sm mx-auto text-left bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div className="flex justify-between">
                  ...
                  <strong className="text-slate-800 uppercase">{selectedKategori.akses === "ilmiah" ? "Sesi Ilmiah (Novotel)" : "Pesta Rakyat (Pekansari)"}</strong>
                </div>
