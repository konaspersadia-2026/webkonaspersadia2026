...
const DRIVE_FOLDER_ID = "1vqz4-BZYYZjIT_4UYrSkufNwXz7fUlDj";

// GANTI DENGAN ID SPREADSHEET ANDA
const SPREADSHEET_ID = "1oYYnHF7nbGgP__-6xMXc8kvc2QyVBy70ZWt_j5KSexc";
    ...
    if (action === "status" && q) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (regId === qClean || email === qClean) {
          ...
          data.akses = rows[i][6];
          data.pilihanKegiatan = rows[i][7];
          data.hargaDasar = rows[i][8];
          data.kodeUnik = rows[i][9];
          data.totalAkhir = rows[i][10];
          data.status = rows[i][11];
          data.noSTR = rows[i][12];
          data.institusi = rows[i][13];
          data.noKTP = rows[i][14];
          data.cabangPersadia = rows[i][15];
          data.slotWaktuCekGula = rows[i][16];
          data.buktiMahasiswaUrl = rows[i][17];
          data.buktiTransferUrl = rows[i][18];
